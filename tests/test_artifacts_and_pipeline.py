...
def _config(tmp_path: Path) -> dict:
    ...
    return {
        ...
        "loader": {"type": "pypdf", "recursive": False, "empty_page_policy": "skip"},
        "chunking": {
...
def test_build_is_immutable_reusable_and_queryable(tmp_path, monkeypatch) -> None:
    ...
    assert first["build_spec"]["build_source_sha256"]
    assert first["source_snapshot_sha256"]
    ...
    assert "index" not in first["artifacts"]
