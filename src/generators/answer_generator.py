...
# OpenAI and deterministic extractive answer generation.
...
from src.text.token_counters import approx_token_count


@dataclass
# 把一次生成结果包装成统一结构，供查询流水线序列化、评估和记录日志。
...
class LLMGenerator:
    ...
    def _optional_usage_value(usage: Any, name: str) -> int | None:
        ...
        if isinstance(value, bool) or not isinstance(value, int) or value < 0:
            raise RuntimeError(
                f"OpenAI usage {name} must be a non-negative integer when present"
            )
        return value
    ...
    def _build_result(
        ...
        if (
            ...
            and provider_total_tokens != provider_input_tokens + provider_output_tokens
        ):
            raise RuntimeError("Provider token totals are inconsistent")
