...
# Safe persistence of evaluation and query results.
...
def write_result_checkpoint(
    ...
    """Atomically persist one redacted evaluation row for exact resume."""

    if not isinstance(row, dict):
        raise TypeError("A result checkpoint must be a dictionary")
    text = json.dumps(
        _safe_value(row),
        ensure_ascii=False,
        allow_nan=False,
